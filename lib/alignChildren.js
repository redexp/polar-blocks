 ...
 * @param {import('types').AlignChildrenOptions} options
 * @returns {import('types').AlignChildrenResult}
 */
export default function alignChildren(
	...
	if (count === 4 && sides.bottom.tryAdd(children)) {
 ...
 * @param {import('types').Block} root
 * @param {import('types').AlignChildrenOptions} options
 * @returns {{left: Side, right: Side, top: Side, bottom: Side}}
 */
...
function alignLeftRight(children, {left, right, bottom}, index = 0) {
	const len = children.length;
